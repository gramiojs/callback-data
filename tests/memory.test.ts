...
import { compareMemoryUsage } from "./utils.ts";

const callbackData = new CallbackData("test").number("secret");
...
test.skipIf(!process.argv.includes("--memory"))("memory safety", async () => {
	...
	expect(bufAfter.arrayBuffers - bufBefore.arrayBuffers).toBeLessThan(
		10 * 1024 * 1024,
	); // <10MB
