...
Поле считается переданным, если его значение **не `undefined`**. Явный `undefined`
(`{ fieldB: undefined }`) эквивалентен полностью опущенному свойству — бит не
выставляется, значение не пишется в строку, результат `pack()` побайтово
совпадает с `pack()` без этого ключа.

При десериализации:
1. Если в данных есть части после required-полей → прочитать bitmask; иначе → bitmask = 0 (все optional отсутствуют)
2. Для каждого `optional[i]`: если бит `i` установлен → прочитать следующую часть; иначе → применить `default` (если есть), а при его отсутствии — не добавлять ключ в результат

Это позволяет безопасно **добавлять optional-поля** в схему — старые данные без bitmask интерпретируются как "все optional отсутствуют".
...
Кнопки в Telegram живут в истории чата — пользователь может нажать кнопку через дни/недели после отправки. Если схема изменилась, старые `callback_data` могут не распарситься.

### Безопасные операции

| Операция | Безопасно? | Почему |
|----------|-----------|--------|
| Добавить optional-поле в конец | Да | Старые данные не содержат bitmask-бита → поле = `undefined` или `default` |
| Добавить default к существующему optional | Да | Только меняет поведение при отсутствии значения |

### Опасные (breaking) операции

| Операция | Что происходит |
|----------|---------------|
| Добавить required-поле | Старые данные не содержат значение → ошибка парсинга |
| Удалить поле | Сдвигает позиции всех последующих полей → мусорные данные |
| Поменять порядок полей | Позиционный формат → значения попадут в чужие поля |
| Сменить тип поля | Десериализация по другому алгоритму → мусор или ошибка |
| Переименовать `nameId` | Меняется 6-char ID → `filter()` не матчит → кнопка "не работает" |

### `safeUnpack()` — защита от крашей

Метод `safeUnpack()` оборачивает `unpack()` и никогда не бросает исключение:

```typescript
const result = schema.safeUnpack(data);
if (result.success) {
    console.log(result.data); // типизированные данные
} else {
    console.log(result.error); // Error с описанием проблемы
    context.answerCallbackQuery("Эта кнопка устарела!");
}
```
...
2. ~~**`console.error` в `deserialize`**~~ — **исправлено**: отсутствующее optional-поле без `default` теперь просто не попадает в результат, без логов ([#3](https://github.com/gramiojs/callback-data/issues/3)).
...
8. **Добавить валидацию на этапе `pack`** — required-поля со значением `undefined` теперь отсекаются на `pack` с сообщением `Missing value for required field '<key>'`. Осталось: неизвестные ключи в объекте по-прежнему молча игнорируются.
